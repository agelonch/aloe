...
inline static void allocate(dft_plan_t *plan, int size_in, int size_out, int len) {
...
void dft_run_c2c(dft_plan_t *plan, dft_c_t *in, dft_c_t *out) {
	...
	copy_in((char*) plan->in,(char*) in,sizeof(dft_c_t),plan->size,plan->options);
	...
	memcpy(out,plan->out,sizeof(dft_c_t)*plan->size);
}
...
void dft_run_r2r(dft_plan_t *plan, dft_r_t *in, dft_r_t *out) {
	...
	copy_in((char*) plan->in,(char*) in,sizeof(dft_r_t),plan->size,plan->options);
	...
	if (plan->options & DFT_PSD) {
		for (i=0;i<(len+1)/2-1;i++) {
			out[i] = f_out[i]*f_out[i]+f_out[len-i-1]*f_out[len-i-1];
		}
	}
	if (plan->options & DFT_OUT_DB) {
		for (i=0;i<len;i++) {
