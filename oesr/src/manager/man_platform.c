 ...
 *
 * 1) Parse "configFile"
 * 2) Call rtdal_create_manager_itfs(configFile)
 * 3) Configure clock:
 *   3.1) If clockLocal==TRUE, call rtdal_attach_local_time() and set sync_local=TRUE
 *   3.2) Otherwise, set sync_itf=rtdal_physicitf_get("sync"); and set sync_local=FALSE
 * 4) For each node
 *   4.1) Configure node, processor and interfaces instances
 *   4.2) set node[i].ctrl_itf=rtdal_itfphysic_get("ctrl_nodename") where nodename is the node name
 *   4.3) set node[i].probe_itf=rtdal_itfphysic_get("probe_nodename") where nodename is the node name 
