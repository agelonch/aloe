 ...
 * accessible by other modules or ALOE (through the oesr_man) for either read or write.
 * On success, oesr_var_create() returns a non-null handler which is passed as a first parameter
 * to oesr_var_close() to close the public variable.
 */
var_t oesr_var_create(void *context, string name, void *ptr, int size) {
