 ...
 *    4.3) if !hwapi.machine.syncContinuous add periodic function SyncRequest() to the kernel
 * with period "period"
 ...
 *    5.2) if hwapi.machine.isContinuous add SyncMaster() as a periodic function to the kernel with
 * period 1
 *    5.3) if !hwapi.machine.isContinuous slaveItf[i].setCallback(SyncMaster,kernelPrio) for each
 * slaveItf
 */
int nod_anode_initialize(int max_waveforms) {
	ndebug("max_waveforms=%d\n",max_waveforms);
	node_packet = &anode.packet;
	if (packet_init(&anode.packet, 5*1024)) {
...
/**
 * This is a thread created by the hwapi kernel thread with normal priority. Reads commands from
